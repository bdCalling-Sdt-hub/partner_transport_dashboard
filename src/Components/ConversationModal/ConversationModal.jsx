    ...
    <div className={`flex items-start space-x-2 ${isSelf ? 'flex-row-reverse text-right' : ''} mb-4`}>
      {
        isSelf ? <img
          ...
          size="large"
          className={isSelf ? 'ml-2 h-10 w-10 rounded-full' : 'mr-2 h-10 w-10 rounded-full'}
        /> : <img
    ...
    <div>
      <Modal onCancel={() => {
        ...
        <div className="p-6  mx-auto bg-white rounded-lg space-y-4 max-h-[80vh] overflow-y-auto">
          {getConversation?.data?.conversation?.messages.map((msg) => (
            <ChatBubble key={msg.id} message={msg} getConversation={getConversation} senderId={senderId} />
          ))}
        </div>
